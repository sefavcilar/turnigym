...
import 'main.dart'; // TurniGymMergedScreen testi için
...
class _MainLayoutState extends State<MainLayout> {
  bool _showInsideCard = true;
  String _listRowCount = '10';
  String _analyticsViewMode = 'GÜNLÜK';
  final TextEditingController _salonCapacityController =
      TextEditingController();
  int _salonCapacity = 50;
  String? _currentLogoUrl;
  final TextEditingController _logoUrlController = TextEditingController();

  String get salonId => FirebaseAuth.instance.currentUser?.uid ?? 'demo_salon';

  // Kurumsal Light Mode konsepti (Dark Mode zorunlu kapatıldı)
  bool get _isDark => false;
  Color get _bgSidebar => Colors.white;
  Color get _bgCard => Colors.white;
  Color get _bgInner => const Color(0xFFF8FAFC);
  Color get _textMain => const Color(0xFF1E293B);
  Color get _textMuted => const Color(0xFF64748B);
  Color get _borderColor => const Color(0xFFE2E8F0);
  Color get _accentColor => const Color(0xFF3B82F6); // Royal Blue Vurgu
  ...
  void initState() {
    ...
    FirebaseFirestore.instance.collection('members').get().then((snapshot) {
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                "Oturumunuzu kapatmak istediğinize emin misiniz? Ana giriş ekranına yönlendirileceksiniz.",
                textAlign: TextAlign.center,
                style: TextStyle(
                  fontSize: 14,
  ...
  Future<bool> _checkIsSuperAdmin() async {
    ...
    final doc = await FirebaseFirestore.instance
        .collection('admins')
        .doc(user.email) // Doküman ID'sini mail adresinle eşleştirdik
        .get();

    if (doc.exists) {
      return doc.get('isSuperAdmin') == true;
    }
    return false;
  }
  ...
  Widget build(BuildContext context) {
    ...
    // 1. Güvenlik Kontrolü: Giriş yapılmadıysa Login sayfasına yolla
    if (user == null) {
      return const LoginView();
    }

    // 2. CEO Seviyesi (Süper Admin) Kontrolü
    if (user.email == "sefavcilar@gmail.com") {
      return _buildSuperAdminView();
    }

    // 3. Normal Salon Sahibi Görünümü
    return _buildSalonOwnerView(context);
  }

  String _formatCiro(double ciro) {
    if (ciro >= 1000000) return "₺${(ciro / 1000000).toStringAsFixed(1)}M";
    if (ciro >= 1000) return "₺${(ciro / 1000).toStringAsFixed(0)}K";
    return "₺${ciro.toStringAsFixed(0)}";
  }

  // Süper Admin'e özel ekran
  Widget _buildSuperAdminView() {
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot>(
        stream: FirebaseFirestore.instance.collection('salonlar').snapshots(),
        builder: (context, snapshot) {
          ...
          final salonlar = snapshot.data?.docs ?? [];

          int toplamSalon = salonlar.length;
          int toplamUye = 0;
          double toplamCiro = 0.0;

          for (var doc in salonlar) {
            var data = doc.data() as Map<String, dynamic>;

            // Veritabanındaki tam alan isimleri ve tip kontrolü
            var uyeDegeri = data['aktifUye'] ?? data['aktifUyeSayisi'];
            var ciroDegeri = data['aylikGelir'];

            if (uyeDegeri != null)
              toplamUye += (uyeDegeri is num) ? uyeDegeri.toInt() : 0;
            if (ciroDegeri != null)
              toplamCiro += (ciroDegeri is num) ? ciroDegeri.toDouble() : 0;
          }
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Expanded(
                      child: _buildCEOStatCard(
                        "TOPLAM SALON",
                        toplamSalon.toString(),
                        Icons.business,
                    ...
                    Expanded(
                      child: _buildCEOStatCard(
                        "TOPLAM CİRO",
                        "${toplamCiro.toStringAsFixed(0)} TL",
                        Icons.payments,
                ...
                Expanded(
                  child: ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      if (index >= salonlar.length)
                        return const SizedBox.shrink();

                      final data =
                          salonlar[index].data() as Map<String, dynamic>;
                      return _buildCleanSalonCard(data, salonlar[index].id);
                    },
                  ),
                ),
              ],
  ...
  Widget _buildCleanSalonCard(Map<String, dynamic> data, String docId) {
    // Çökmeyi önlemek için tip güvenliği ve .toString() koruması
    final salonName = (data['salonAdi'] ?? 'İsimsiz Salon').toString();
    final city = (data['city'] ?? 'İstanbul').toString();
    final memberCount = (data['aktifUye'] ?? data['aktifUyeSayisi'] ?? 0)
        .toString();
    final aylikGelirRaw = data['aylikGelir'];
    final revenue = aylikGelirRaw is num
        ? _formatCiro(aylikGelirRaw.toDouble())
        : (aylikGelirRaw?.toString() ?? '₺0');
        ...
        children: [
          ...
          Container(
            ...
            child: IconButton(
              ...
              onPressed: () {
                // Gelecekte salona giriş yap veya detayını gör işlevi eklenebilir
              },
            ),
          ),
        ],
  ...
  Widget _buildSalonOwnerView(BuildContext context) {
                ...
                children: [
                  ...
                  Expanded(
                    child: _selectedIndex == 0
                        ...
                        : _selectedIndex == 2
                        ? _buildPackageStoreView()
                        : _selectedIndex == 3
                        ? _buildSalesReportsView()
                        : _selectedIndex == 4
                        ? _buildPassAnalyticsView()
                        : _selectedIndex == 5
                        ? _buildCompanyManagementView()
                        : _selectedIndex == 6
                        ? _buildHardwareCalibrationView()
                        : _selectedIndex == 7
                              ...
                              try {
                                ...
                                if (memberSnapshot.docs.isNotEmpty) {
                                  ...
                                  if (currentCredit > 0) {
                                    ...
                                    if (mounted)
                                      ScaffoldMessenger.of(
                                        ...
                                        SnackBar(
                                          content: Text(
                                            '❌ $memberName isimli üyenin kredisi yetersiz!',
                                          ),
                                  ...
                                  if (mounted)
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      const SnackBar(
                                        content: Text('⚠️ Kayıtlı üye yok.'),
                                      ),
                                    );
                                }
                              } catch (e) {
                          ...
                          child: StreamBuilder<QuerySnapshot>(
                            ...
                            builder: (context, snapshot) {
                              ...
                              filteredDocs.sort((a, b) {
                                ...
                                return (dataB['timestamp'] ?? 0).compareTo(
                                  dataA['timestamp'] ?? 0,
                                );
                              });
  ...
  Widget _buildCyberTableRow(
    ...
    return Material(
      color: _isDark
          ...
          : (index % 2 == 0 ? AppColors.white : const Color(0xFFF1F5F9)),
      borderRadius: BorderRadius.circular(12),
                      ...
                      : [
                          ...
                          ElevatedButton(
                            onPressed: () async {
                              ...
                              await _performSale(memberId, price, name);
          ...
          StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, snapshot) {
              ...
              return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  if (index >= docs.length) return const SizedBox.shrink();
            ...
            children: [
              ...
              SegmentedButton<String>(
                ...
                onSelectionChanged: (newSelection) =>
                    setState(() => _analyticsViewMode = newSelection.first),
              ),
            ],
          ),
          const SizedBox(height: 24),
  ...
  Future<void> _updateLogoUrl() async {
    try {
      ...
      if (mounted) {
        setState(() => _currentLogoUrl = newUrl.isEmpty ? null : newUrl);
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
                ...
                children: [
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        const Text(
                          "Şifre yenileme işlemi, yüksek güvenlik gerektirdiği için 3 adımlı doğrulama ile yapılmaktadır.",
                          style: TextStyle(
                            color: Color(0xFF627E82),
                            fontSize: 13,
                          ),
...
class _ChangePasswordDialogState extends State<ChangePasswordDialog> {
  ...
  Future<void> _updatePassword() async {
    ...
    try {
      ...
      String errorMsg = e.message ?? "Bir hata oluştu.";
